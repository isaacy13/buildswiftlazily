...
- Optional `BSL_KEYCHAIN_PASSWORD` unlocks the login keychain for local `xcodebuild` after sleep — treat it like the Mac login secret (`.env` only, never the browser). Prefer `./scripts/prepare-keychain.sh` so codesign ACLs do not need a GUI prompt. When set, `build-ios.sh` unlocks then **unsets** the variable before `xcodebuild` so Run Script phases do not inherit it; job logs also redact it.
